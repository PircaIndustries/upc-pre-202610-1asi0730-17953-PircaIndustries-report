...
| :--- | :--- | :--- | :--- | :--- |
| **TS01** | Create API User | **Como** usuario developer, **quiero** diseñar una API que facilite la gestión de usuarios, **para** administrar eficazmente la seguridad y acceso a la plataforma. | **Escenario 1: Diseño de la API User** <br>Dado que el usuario developer configura la plataforma, <br>Cuando diseñe la API para gestionar usuarios, <br>Entonces definirá los endpoints y rutas para registro e inicio de sesión estableciendo el estándar de seguridad. | **EP01** |
| **TS02** | Post User | **Como** usuario developer, **quiero** registrar usuarios en el API, **para** que los profesionales puedan crear sus cuentas de obra. | **Escenario 1: Agregar User** <br>Dado que el endpoint "/usuarios" está disponible, <br>Cuando se envía una solicitud POST con los detalles del usuario, <br>Entonces se recibe una respuesta con estado 201 y los detalles registrados. <br><br>**Escenario 2: Agregar User ya existente** <br>Dado que el endpoint "/usuarios" está disponible, <br>Cuando se envía una solicitud POST con un correo ya registrado, <br>Entonces se recibe una respuesta con estado 400. | **EP01** |
| **TS03** | Get User | **Como** usuario developer, **quiero** obtener la información de un usuario, **para** usarla en la aplicación cuando se necesite mostrar el perfil. | **Escenario 1: Obtener informacion de User** <br>Dado que el endpoint "/usuarios" está disponible, <br>Cuando se envía una solicitud GET con el identificador del usuario, <br>Entonces se recibe una respuesta con estado 200 y los datos del usuario. <br><br>**Escenario 2: Obtener informacion de User no disponible** <br>Dado que el endpoint "/usuarios" está disponible, <br>Cuando se envía una solicitud GET con un ID inexistente, <br>Entonces se recibe una respuesta con estado 404. | **EP01** |
| **TS04** | Create API Projects | **Como** usuario developer, **quiero** diseñar una API para la gestión de proyectos de obra, **para** centralizar la información técnica de las construcciones. | **Escenario 1: Diseño de la API Projects** <br>Dado que el developer configura el módulo de obra, <br>Cuando diseñe la API de proyectos, <br>Entonces definirá las rutas para creación y listado de obras asegurando la integridad de los datos financieros. | **EP02** |
| **TS05** | Post Project | **Como** usuario developer, **quiero** registrar nuevas obras en el API, **para** iniciar el seguimiento operativo del proyecto. | **Escenario 1: Agregar Project** <br>Dado que el endpoint "/proyectos" está disponible, <br>Cuando se envía una solicitud POST con el nombre y presupuesto asignado, <br>Entonces se recibe una respuesta con estado 201 y el nuevo ID generado. <br><br>**Escenario 2: Agregar Project ya existente** <br>Dado que el endpoint "/proyectos" está disponible, <br>Cuando se intenta registrar un proyecto con un código de obra duplicado, <br>Entonces se recibe una respuesta con estado 400. | **EP02** |
| **TS06** | Get Project | **Como** usuario developer, **quiero** consultar la lista de obras, **para** mostrar los indicadores de avance en la aplicación. | **Escenario 1: Obtener informacion de Project** <br>Dado que el endpoint "/proyectos" está disponible, <br>Cuando se envía una solicitud GET, <br>Entonces se recibe una respuesta con estado 200 y la lista de proyectos. <br><br>**Escenario 2: Obtener informacion de Project no disponible** <br>Dado que el endpoint "/proyectos" está disponible, <br>Cuando no existen proyectos registrados en la base de datos, <br>Entonces se recibe una respuesta con estado 200 y un arreglo vacío. | **EP02** |
...
| **TS08** | Post Advance | **Como** usuario developer, **quiero** registrar el progreso diario en el API, **para** actualizar el porcentaje de avance físico frente al cronograma. | **Escenario 1: Agregar Advance** <br>Dado que el endpoint "/avances" está disponible, <br>Cuando se envía una solicitud POST con un valor entre 1 y 100, <br>Entonces se recibe una respuesta con estado 201 confirmando el progreso. <br><br>**Escenario 2: Agregar Advance ya existente** <br>Dado que el endpoint "/avances" está disponible, <br>Cuando se envía un avance para una fecha y partida que ya cuenta con registro, <br>Entonces se recibe una respuesta con estado 400. | **EP02** |
| **TS09** | Get Advance | **Como** usuario developer, **quiero** consultar los avances registrados, **para** calcular el Porcentaje de Plan Completado (PPC). | **Escenario 1: Obtener informacion de Advance** <br>Dado que el endpoint "/avances" está disponible, <br>Cuando se envía una solicitud GET especificando el proyecto, <br>Entonces se recibe una respuesta con estado 200 y los porcentajes acumulados. <br><br>**Escenario 2: Obtener informacion de Advance no disponible** <br>Dado que el endpoint "/avances" está disponible, <br>Cuando se consulta un proyecto que aún no tiene avances registrados, <br>Entonces se recibe una respuesta con estado 404. | **EP02** |
| **TS10** | Create API RNC | **Como** usuario developer, **quiero** diseñar una API para incidencias no conformes, **para** gestionar los fallos de calidad detectados en campo. | **Escenario 1: Diseño de la API RNC** <br>Dado que el developer configura el módulo de calidad, <br>Cuando diseñe la API de RNC, <br>Entonces definirá las rutas para reporte de fallas y almacenamiento de rutas de evidencia fotográfica. | **EP02** |
| **TS11** | Post RNC | **Como** usuario developer, **quiero** registrar fallas técnicas en el API, **para** alertar inmediatamente al equipo de diseño. | **Escenario 1: Agregar RNC** <br>Dado que el endpoint "/rnc" está disponible, <br>Cuando se envía una solicitud POST con la descripción y evidencia, <br>Entonces se recibe una respuesta con estado 201 y un nuevo ID de ticket. <br><br>**Escenario 2: Agregar RNC ya existente** <br>Dado que el endpoint "/rnc" está disponible, <br>Cuando se intenta registrar una falla con un identificador de referencia técnica duplicado, <br>Entonces se recibe una respuesta con estado 400. | **EP02** |
| **TS12** | Get RNC | **Como** usuario developer, **quiero** obtener la lista de RNC, **para** permitir la revisión y subsanación de errores de obra. | **Escenario 1: Obtener informacion de RNC** <br>Dado que el endpoint "/rnc" está disponible, <br>Cuando se envía una solicitud GET con el ID de obra, <br>Entonces se recibe una respuesta con estado 200 y los detalles de los RNC pendientes. <br><br>**Escenario 2: Obtener informacion de RNC no disponible** <br>Dado que el endpoint "/rnc" está disponible, <br>Cuando se consulta un ID de obra sin incidencias de calidad, <br>Entonces se recibe una respuesta con estado 200 y una lista vacía. | **EP02** |
| **TS13** | Create API Materials | **Como** usuario developer, **quiero** diseñar una API para el control logístico, **para** administrar el stock de materiales de construcción. | **Escenario 1: Diseño de la API Materials** <br>Dado que el developer configura el módulo logístico, <br>Cuando diseñe la API de materiales, <br>Entonces establecerá los endpoints necesarios para actualización de inventario y pedidos de insumos. | **EP03** |
| **TS14** | Post Material | **Como** usuario developer, **quiero** registrar pedidos de materiales en el API, **para** que el área de compras procese los requerimientos. | **Escenario 1: Agregar Material** <br>Dado que el endpoint "/pedidos" está disponible, <br>Cuando se envía una solicitud POST con la cantidad requerida, <br>Entonces se recibe una respuesta con estado 201. <br><br>**Escenario 2: Agregar Material ya existente** <br>Dado que el endpoint "/pedidos" está disponible, <br>Cuando se envía una solicitud de pedido duplicada para el mismo material y lote en la misma sesión, <br>Entonces se recibe una respuesta con estado 400. | **EP03** |
| **TS15** | Get Material | **Como** usuario developer, **quiero** consultar el stock disponible en tiempo real, **para** informar al personal sobre la existencia de materiales. | **Escenario 1: Obtener informacion de Material** <br>Dado que el endpoint "/materiales" está disponible, <br>Cuando se envía una solicitud GET, <br>Entonces se recibe una respuesta con estado 200 y las cantidades actuales. <br><br>**Escenario 2: Obtener informacion de Material no disponible** <br>Dado que el endpoint "/materiales" está disponible, <br>Cuando se consulta por un material específico que no existe en el catálogo, <br>Entonces se recibe una respuesta con estado 404. | **EP03** |
| **TS16** | Create API Budget | **Como** usuario developer, **quiero** diseñar una API financiera, **para** controlar que los gastos no excedan el presupuesto meta. | **Escenario 1: Diseño de la API Budget** <br>Dado que el developer configura el módulo financiero, <br>Cuando diseñe la API de presupuesto, <br>Entonces definirá las reglas de negocio para validar saldos antes de confirmar cualquier compra. | **EP03** |
| **TS17** | Post Budget | **Como** usuario developer, **quiero** cargar el presupuesto inicial en el API, **para** establecer los topes económicos de la obra. | **Escenario 1: Agregar Budget** <br>Dado que el endpoint "/presupuesto" está disponible, <br>Cuando se envía una solicitud POST con los montos por partida, <br>Entonces se recibe una respuesta con estado 201. <br><br>**Escenario 2: Agregar Budget ya existente** <br>Dado que el endpoint "/presupuesto" está disponible, <br>Cuando se intenta cargar un presupuesto inicial para una obra que ya tiene fondos asignados, <br>Entonces se recibe una respuesta con estado 400. | **EP03** |
| **TS18** | Get Budget | **Como** usuario developer, **quiero** obtener el estado del presupuesto, **para** visualizar desviaciones financieras en los paneles de control. | **Escenario 1: Obtener informacion de Budget** <br>Dado que el endpoint "/presupuesto" está disponible, <br>Cuando se envía una solicitud GET, <br>Entonces se recibe una respuesta con estado 200 con el saldo consumido y disponible. <br><br>**Escenario 2: Obtener informacion de Budget no disponible** <br>Dado que el endpoint "/presupuesto" está disponible, <br>Cuando se consulta el estado financiero de un proyecto sin presupuesto definido, <br>Entonces se recibe una respuesta con estado 404. | **EP03** |
