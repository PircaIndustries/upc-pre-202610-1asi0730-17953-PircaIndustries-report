...
| :--- | :--- | :--- | :--- | :--- |
| **US-01** | Registro de cuenta nueva | Como nuevo miembro del equipo, quiero registrarme en la plataforma para acceder a los datos del proyecto asignado. | **Escenario 1:** <br>Dado que el usuario accede por primera vez a la aplicación, <br>Cuando completa el formulario de registro con sus datos, <br>Entonces el sistema crea la cuenta y lo redirige al Dashboard. <br><br>**Escenario 2:** <br>Dado que el usuario intenta registrarse con un correo ya existente, <br>Cuando completa el formulario y envía los datos, <br>Entonces el sistema muestra un mensaje de error indicando que la cuenta ya está registrada. | **EP04** |
| **US-02** | Visualización en "Modo Obra" | Como residente, quiero activar el tema de alto contraste para visualizar datos bajo luz solar intensa sin fatiga. | **Escenario 1:** <br>Dado que el usuario se encuentra en una inspección de campo, <br>Cuando activa el interruptor de "Modo Obra" en los ajustes, <br>Entonces la interfaz cambia a colores oscuros con fuentes de alta visibilidad. <br><br>**Escenario 2:** <br>Dado que el usuario ya no está expuesto a luz solar intensa, <br>Cuando desactiva el interruptor de "Modo Obra", <br>Entonces la interfaz regresa al tema estándar de la aplicación. | **EP03** |
| **US-03** | Registro de incidencia RNC | Como supervisor, quiero registrar un Resultado No Conforme con fotos para que el error sea subsanado de inmediato. | **Escenario 1:** <br>Dado que el usuario detecta una falla técnica en la estructura, <br>Cuando sube la descripción y la evidencia fotográfica al módulo RNC, <br>Entonces el sistema genera un ticket de alerta para el equipo de diseño. <br><br>**Escenario 2:** <br>Dado que el usuario olvidó adjurar una foto obligatoria, <br>Cuando intenta enviar el formulario RNC sin imágenes, <br>Entonces el sistema bloquea el envío y muestra un mensaje solicitando al menos una foto. | **EP01** |
| **US-04** | Solicitud formal de materiales | Como residente, quiero enviar pedidos de insumos desde la app para evitar errores de comunicación por canales informales. | **Escenario 1:** <br>Dado que el usuario requiere materiales para una partida específica, <br>Cuando completa el formulario de requerimiento técnico, <br>Entonces el ticket se envía al área de logística con un código de seguimiento. <br><br>**Escenario 2:** <br>Dado que el usuario necesita modificar una solicitud ya enviada, <br>Cuando accede al historial de pedidos y edita los campos antes de que sea procesado, <br>Entonces el sistema actualiza el ticket y notifica el cambio a logística. | **EP02** |
| **US-05** | Alerta automática de stock crítico | Como encargado de almacén, quiero recibir avisos cuando el inventario sea bajo para evitar paralizar la construcción. | **Escenario 1:** <br>Dado que el usuario registra la salida de un material que llega a su límite mínimo, <br>Cuando el sistema procesa el despacho, <br>Entonces el indicador visual cambia a rojo y se emite una notificación de reposición. <br><br>**Escenario 2:** <br>Dado que el stock de un material cae por debajo del mínimo crítico durante la noche, <br>Cuando el sistema ejecuta la verificación automática de inventario, <br>Entonces envía una alerta push al encargado y al jefe de compras. | **EP02** |
| **US-06** | Firma digital de actas (Token) | Como ingeniero residente, quiero firmar actas de conformidad con un token para asegurar la validez legal del cierre de etapa. | **Escenario 1:** <br>Dado que el usuario verifica que la partida cumple los estándares, <br>Cuando ingresa el código de verificación enviado a su celular, <br>Entonces el documento PDF se sella digitalmente como aprobado. <br><br>**Escenario 2:** <br>Dado que el usuario ingresa un código de verificación incorrecto, <br>Cuando intenta validar la firma digital, <br>Entonces el sistema rechaza la operación y muestra un mensaje de error sin sellar el documento. | **EP01** |
...
| **US-08** | Filtro de planos por especialidad | Como técnico, quiero filtrar los planos por tipo (Sanitarias, Eléctricas, etc.) para localizar el diseño vigente rápidamente. | **Escenario 1:** <br>Dado que el usuario visualiza la lista de documentos técnicos, <br>Cuando selecciona el filtro de la especialidad requerida, <br>Entonces la aplicación muestra únicamente los planos correspondientes a esa categoría. <br><br>**Escenario 2:** <br>Dado que el usuario aplica múltiples filtros simultáneamente (especialidad + versión vigente), <br>Cuando confirma la selección, <br>Entonces el sistema muestra solo los planos que cumplen todas las condiciones. | **EP03** |
| **US-09** | Cálculo automático de KPI (PPC) | Como gerente, quiero visualizar el Porcentaje de Plan Completado semanal para medir la productividad real de la obra. | **Escenario 1:** <br>Dado que el usuario ha marcado las tareas del día como finalizadas, <br>Cuando accede al reporte de desempeño semanal, <br>Entonces el sistema muestra gráficamente el porcentaje de avance logrado frente al planeado. <br><br>**Escenario 2:** <br>Dado que el usuario necesita ver la evolución del PPC en el último mes, <br>Cuando selecciona el filtro de período en el módulo de indicadores, <br>Entonces el sistema genera una tendencia semanal con los valores históricos. | **EP01** |
| **US-10** | Historial de cambios en planos | Como arquitecto, quiero ver el registro de versiones de un plano para asegurar que el equipo no use diseños obsoletos. | **Escenario 1:** <br>Dado que el usuario consulta los detalles de un plano específico, <br>Cuando abre la sección de historial de archivos, <br>Entonces el sistema lista todas las versiones previas con su respectiva fecha y responsable. <br><br>**Escenario 2:** <br>Dado que el usuario necesita restaurar una versión anterior del plano, <br>Cuando selecciona una versión previa y confirma la restauración, <br>Entonces el sistema actualiza el plano vigente y registra el cambio en el historial. | **EP03** |
| **US-11** | Landing page - Contenido para gestores operativos | Como arquitecto o ingeniero que visita el sitio, quiero encontrar información específica sobre cómo Kipu mejora la supervisión técnica y trazabilidad de obra para evaluar si se adapta a mis necesidades profesionales. | **Escenario 1:** <br>Dado que un gestor operativo accede al landing page, <br>Cuando navega a la sección "Para profesionales de obra", <br>Entonces el sitio muestra contenido específico sobre gestión de RNC, seguimiento de PPC y firma digital de actas. <br><br>**Escenario 2:** <br>Dado que el gestor operativo está interesado en la solución, <br>Cuando hace clic en el botón "Ver más" o "Comenzar" dentro de esa sección, <br>Entonces es redirigido al formulario de registro de la web application. | **EP01** |
| **US-12** | Landing page - Contenido para equipos de logística | Como responsable de logística o administración que visita el sitio, quiero encontrar información específica sobre cómo Kipu centraliza pedidos y controla presupuestos para convencerme de su utilidad en mi área. | **Escenario 1:** <br>Dado que un profesional de logística accede al landing page, <br>Cuando navega a la sección "Para equipos de logística y administración", <br>Entonces el sitio muestra contenido específico sobre solicitud de materiales, alertas de stock crítico y validación de compras vs presupuesto. <br><br>**Escenario 2:** <br>Dado que el profesional de logística está interesado en la solución, <br>Cuando hace clic en el botón "Solicitar demo" dentro de esa sección, <br>Entonces es redirigido a un formulario de contacto o registro según corresponda. | **EP01** |
| **US-13** | Subir nueva versión de plano | Como Residente de Obra,<br>Quiero subir una nueva versión actualizada de un plano técnico,<br>Para asegurar que el equipo en campo construya basándose en el diseño final aprobado. | **Escenario 1: Subida de nueva versión exitosa**<br>Dado que la sección de expedientes del proyecto está abierta,<br>Cuando se adjunta un archivo PDF válido y se confirma la subida,<br>Entonces el sistema etiqueta el nuevo documento como "Versión Actual",<br>Y archiva automáticamente el documento anterior en el historial.<br><br>**Escenario 2: Archivo con formato inválido**<br>Dado que la sección de expedientes está abierta,<br>Cuando se intenta subir un archivo con extensión no permitida (ej. .exe o .mp4),<br>Entonces el sistema muestra un mensaje de error de formato,<br>Y cancela el proceso de subida de versión. | EP01 |
| **US-14** | Consultar historial de planos | Como Residente de Obra,<br>Quiero visualizar el historial completo de versiones de un plano,<br>Para auditar los cambios realizados por el área de diseño a lo largo del tiempo. | **Escenario 1: Visualización de historial**<br>Dado que existe un plano con múltiples versiones cargadas,<br>Cuando se selecciona la opción "Ver historial",<br>Entonces se despliega una lista ordenada cronológicamente con todas las versiones,<br>Y se habilita la opción de descargar cada versión anterior. | EP01 |
| **US-15** | Generar requerimiento de materiales | Como Residente de Obra,<br>Quiero crear una solicitud formal de materiales desde la plataforma,<br>Para que el equipo de logística proceda con las compras sin utilizar canales informales. | **Escenario 1: Creación de solicitud exitosa**<br>Dado que el formulario de requerimiento de materiales está disponible,<br>Cuando se completan los campos obligatorios (material, cantidad, fecha límite) y se envía,<br>Entonces el sistema registra la solicitud con estado "Pendiente",<br>Y emite una notificación al Encargado de Logística.<br><br>**Escenario 2: Formulario incompleto**<br>Dado que el formulario de requerimiento de materiales está abierto,<br>Cuando se envía el formulario omitiendo la cantidad requerida,<br>Entonces el sistema resalta el campo en rojo,<br>Y muestra una alerta indicando que los campos obligatorios deben completarse. | EP02 |
| **US-16** | Atender solicitud de materiales | Como Encargado de Logística,<br>Quiero cambiar el estado de las solicitudes de materiales a "Aprobado" o "Rechazado",<br>Para mantener al equipo de obra informado sobre la disponibilidad de los insumos. | **Escenario 1: Aprobación de requerimiento**<br>Dado que existe una solicitud de material en estado "Pendiente",<br>Cuando se selecciona la solicitud y se marca como "Aprobada",<br>Entonces el estado de la solicitud cambia a "En proceso de compra",<br>Y el Residente de Obra visualiza el cambio de estado en su panel. | EP02 |
| **US-17** | Actualizar inventario general | Como Encargado de Logística,<br>Quiero actualizar las cantidades de los materiales en el almacén central,<br>Para mantener un registro exacto del stock físico disponible para las obras. | **Escenario 1: Ingreso de nuevo stock**<br>Dado que el módulo de inventario está accesible,<br>Cuando se ingresa una cantidad numérica positiva en el registro de un material existente,<br>Entonces el sistema suma el valor ingresado al stock actual,<br>Y registra la fecha y usuario responsable de la modificación. | EP03 |
| **US-18** | Visualizar alerta de stock bajo (cualquier material crítico) | Como Encargado de Logística, quiero recibir una alerta visual cuando cualquier material crítico (ej. acero, cemento) alcance su nivel mínimo, para generar órdenes de compra antes de que ocurra un quiebre de stock. | **Escenario 1: Disparador de alerta de stock**<br>Dado que un material (ej. acero, cemento, ladrillos) tiene configurado un límite mínimo,<br>Cuando el stock registrado desciende por debajo de ese límite,<br>Entonces el sistema marca la fila del material en color rojo en la tabla de inventario,<br>Y activa un ícono de advertencia en el Dashboard principal. | EP03 |
| **US-19** | Registrar avance diario de obra | Como Residente de Obra,<br>Quiero registrar el porcentaje de avance físico del proyecto diariamente,<br>Para mantener la trazabilidad de la ejecución real frente al cronograma. | **Escenario 1: Registro de avance porcentual**<br>Dado que el panel de control del proyecto está activo,<br>Cuando se ingresa un porcentaje de avance válido (1-100) y se adjunta una observación,<br>Entonces el sistema actualiza la barra de progreso general del proyecto,<br>Y guarda el registro en la bitácora histórica de la obra. | EP04 |
| **US-20** | Cambiar estado del proyecto | Como Administrador del Sistema,<br>Quiero modificar el estado global de una obra (ej. En ejecución, Paralizada, Finalizada),<br>Para reflejar la situación operativa actual ante todos los usuarios. | **Escenario 1: Paralización de obra**<br>Dado que un proyecto se encuentra en estado "En ejecución",<br>Cuando se selecciona el estado "Paralizada" y se confirma el cambio,<br>Entonces el sistema actualiza el distintivo visual del proyecto,<br>Y bloquea la creación de nuevos requerimientos de materiales para dicha obra. | EP04 |
| **US-21** | Visualizar Dashboard logístico | Como Encargado de Logística,<br>Quiero visualizar un resumen gráfico de requerimientos pendientes y presupuesto ejecutado,<br>Para tomar decisiones de compra rápidas sin tener que cruzar datos de Excel. | **Escenario 1: Carga de indicadores principales**<br>Dado que el usuario inicia sesión con el rol de Logística,<br>Cuando se carga la vista principal (Dashboard),<br>Entonces el sistema renderiza un gráfico de anillos con las solicitudes pendientes,<br>Y muestra una tarjeta con el monto total de presupuesto ejecutado en el mes. | EP05 |
| **US-22** | Asignar roles a usuarios | Como Administrador del Sistema,<br>Quiero asignar o modificar los permisos de los trabajadores en la plataforma,<br>Para asegurar que cada empleado solo tenga acceso a los módulos correspondientes a su cargo. | **Escenario 1: Asignación de rol de Logística**<br>Dado que existe un usuario registrado con acceso básico,<br>Cuando se le asigna el rol "Encargado de Logística" desde el panel de administración,<br>Entonces el sistema habilita el acceso a los módulos de Inventario y Requerimientos para ese usuario en su próximo inicio de sesión. | EP06 |
...
| **US-24** | Notificaciones automáticas de nuevas versiones de planos | Como residente de obra, quiero recibir una alerta automática (push y email) cuando el arquitecto suba una nueva versión de un plano, para evitar continuar la construcción con diseños obsoletos. | **Escenario 1:** <br>Dado que el arquitecto sube una nueva versión de un plano (US-13), <br>Cuando el sistema procesa la actualización, <br>Entonces envía una notificación push y un correo electrónico a todos los usuarios que hayan visualizado ese plano previamente. <br><br>**Escenario 2:** <br>Dado que un usuario tiene desactivadas las notificaciones push en su perfil, <br>Cuando se genera una nueva versión de plano, <br>Entonces el sistema solo envía el correo electrónico. | **EP03** |
| **US-25** | Exportación de Dossier de Calidad en PDF | Como administrador de proyecto, quiero exportar un archivo PDF que contenga todo el historial de firmas electrónicas y aprobaciones de una etapa constructiva, para facilitar la entrega del proyecto a la municipalidad o al cliente final. | **Escenario 1:** <br>Dado que el usuario accede al módulo de una etapa finalizada, <br>Cuando selecciona "Exportar Dossier de Calidad", <br>Entonces el sistema genera un PDF con todas las actas firmadas digitalmente (US-06) y su línea de tiempo de aprobaciones. <br><br>**Escenario 2:** <br>Dado que el usuario necesita incluir solo ciertas actas, <br>Cuando aplica un filtro por fechas o tipo de documento antes de exportar, <br>Entonces el PDF generado contiene únicamente los documentos seleccionados. | **EP01** |
